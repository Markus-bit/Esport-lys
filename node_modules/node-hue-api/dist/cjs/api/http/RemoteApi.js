...
class RemoteApi {
    ...
    getDigestResponse(realm, nonce, method, path) {
        const clientId = this.clientId, clientSecret = this.clientSecret, hashOne = crypto_1.createHash('md5').update(`${clientId}:${realm}:${clientSecret}`).digest('hex'), hashTwo = crypto_1.createHash('md5').update(`${method.toUpperCase()}:${path}`).digest('hex'), hash = crypto_1.createHash('md5').update(`${hashOne}:${nonce}:${hashTwo}`).digest('hex');
        if (!clientId) {
