...
const stateTypes_1 = __importDefault(require("./stateTypes"));
const PERCENTAGE = new types_1.UInt8Type({ name: 'percentage', min: 0, max: 100 }), DEGREES = new types_1.UInt8Type({ name: 'degrees', min: 0, max: 360 });
class States {
    constructor(attributes) {
        const states = {};
        attributes.forEach(attribute => {
            // @ts-ignore
            const parameter = stateTypes_1.default[attribute];
            if (!parameter) {
                throw new HueBridgeModelError_1.HueBridgeModelError(`Unknown Light State Parameter: "${attribute}"`);
            }
    ...
    _setStateValue(definitionName, value) {
        ...
        if (stateDefinition) {
            ...
            throw new HueBridgeModelError_1.HueBridgeModelError(`Attempted to set a state '${definitionName}' that is not one of the allowed states`);
        }
        return this.me;
    }
